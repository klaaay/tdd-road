...
function useAntdTable<Result, Item>(
  ...
  const _deps: DependencyList = (Array.isArray(deps)
    ? deps
    : []) as DependencyList;
  const _options: Options<Result, Item> = (typeof deps === "object" &&
  !Array.isArray(deps)
    ? deps
  ...
  const [state, dispatch] = useReducer<Reducer<UseTableInitState<Item>, any>>(
    reducer,
    {
      ...initState,
  ...
  const changeSearchType = useCallback(() => {
    ...
    const targetSearchType =
      state.searchType === "simple" ? "advance" : "simple";
    dispatch({
      type: "updateState",
      payload: {
